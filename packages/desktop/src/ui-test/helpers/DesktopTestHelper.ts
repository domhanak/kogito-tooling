...
export default class DesktopTestHelper {
    ...
    public switchToFrame = async (frameId: string): Promise<void> => {
      const frameElement = await this.testedApplication.client.$("#" + frameId);
      expect(await (frameElement).waitForExist()).toBeTruthy();
      this.testedApplication.client.switchToFrame(frameElement);
    }
    ...
    public switchToParentFrame = async (): Promise<void> => {
        this.testedApplication.client.switchToParentFrame();
    }
